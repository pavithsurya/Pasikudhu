...
class EditUserReview extends StatefulWidget {
  static const String id = 'edit-user-review';
  const EditUserReview({Key? key}) : super(key: key);

  @override
  State<EditUserReview> createState() => _EditUserReviewState();
}
...
class _EditUserReviewState extends State<EditUserReview> {
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 10,),
                Text('Are you sure want to edit the item?'),
                SizedBox(height: 10,),
                ListTile(
                ...
                Row(
                  children: [
                    ...
                    IconButton(onPressed: (){
                      Navigator.push(context, MaterialPageRoute(builder: (BuildContext context)=> LocationScreen(true,EditUserReview())));
  ...
  Future<void> saveItemtoDB(CategoryProvider provider,context) async {
    var _prodprovider = Provider.of<ProductProvider>(context,listen: false);
    var data = _prodprovider.productdata;
    print('yes');

    return _service.items.doc(data.id)
        .update(
      provider.dataUpload,
    )
        .then((value) {
      provider.clearData();
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Your Ad is updated successfully"))
      );
